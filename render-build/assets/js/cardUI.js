...
(function () {
  ...
  app.addSlash = function (e) {
    let isMonthEntered = app.monthRegex.exec(e.target.value);
  ...
  app.removeSlash = function (e) {
    let isMonthAndSlashEntered = app.monthAndSlashRegex.exec(e.target.value);
  ...
  app.checkMonth = function (month) {
    if (month <= 12) {
      let monthSelectOptions = app.getSelectOptions($(app.el_monthSelect));
  ...
  app.checkYear = function (year) {
    let yearSelectOptions = app.getSelectOptions($(app.el_yearSelect));
  ...
  app.formatCardNumber = function (cc_num, card_type) {
    ...
    let cardFormatString = app.getCardFormatString(cc_num, card_type);
  ...
  app.monitorCcFormat = function ($elem) {
    let cc_num = $elem.val().replace(/\D/g, "");
    let card_type = app.getCardType(cc_num);
  ...
  app.addCardClassIdentifier = function ($elem, card_type) {
    let classIdentifier = app.el_ccUnknown;
    if (card_type) {
